    ...
    for result in results:
        for box in result.boxes:
            ...
            if i == 0 :
                ...
                output.append([i,center_w,center_h,int(box.xyxy[0][2]) - int(box.xyxy[0][0]),int(box.xyxy[0][1]) - int(box.xyxy[0][0])])
                output.append([int(box.xyxy[0][0]), int(box.xyxy[0][1]),int(box.xyxy[0][2]), int(box.xyxy[0][3])])
...
frame = cv2.imread("/home/song/CoopProject/img/380945_0.jpg")
height, width, channels = frame.shape
print(height, width, channels )
while True:
    img,results = predict_and_detect(model,frame)
    print(results)
    print((1753.8*(449.5/results[0][3])+7.3749)/100)
